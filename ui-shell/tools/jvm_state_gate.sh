...
fun main(){
 ...
 val queueResume=ShellState(f);queueResume.selectTrack(book,true);queueResume.seekFraction(.71f);val queuedSaved=queueResume.positionMs;val bookIndex=queueResume.currentIndex;queueResume.removeFromQueue(bookIndex);queueResume.selectTrack(book,false);ok(queueResume.positionMs==queuedSaved,"longform queue-removal resume")
 queueResume.togglePlayedLongform(book);ok(book.id in queueResume.playedLongform && queueResume.resumePositions[book.id]==book.durationMs,"longform played completion");queueResume.togglePlayedLongform(book);ok(book.id !in queueResume.playedLongform && queueResume.resumePositions[book.id]==0L && queueResume.positionMs==0L,"longform unplayed reset")
