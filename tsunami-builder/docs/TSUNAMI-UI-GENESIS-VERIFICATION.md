...
## Current-head public mirror handoff — 2026-09-26 16:23 UTC

The isolated public builder mirror in `brass-crusader-tx/chromagora-os` was rebound to the exact current Genesis head `b45c93572c3eccf987af21629f712f42daeedbd1` rather than the earlier `59df4c30…` snapshot. A tree-level comparison after the sync found **40 / 40 ui-shell blobs and 16 / 16 support blobs byte-identical** between the private Genesis source and `tsunami-builder/`; mismatches: **0**. The public mirror commit carrying that binding is `50832554791f774ae0e9416fc545b7ace0e78a26`.

That push triggered two independent builders. GitHub-hosted run `36255294135` again failed before any step was created (job `108440804742`, `steps=null`), preserving the existing hosted-runner-allocation diagnosis. The separate self-hosted run `36255294100` accepted the same current-head mirror and is queued for the registered self-hosted executor (job `108440806554`). This queued state is not build evidence; it is, however, a current-head executable handoff with exact-source binding ready to produce the canonical APK/device evidence as soon as that executor is available.


## Exact-current-head reconciliation — 2026-09-26 17:4x UTC

The isolated Genesis branch is currently at `46fd4b61915936187521a3e0c5f542505b315c48`. A fresh base-to-head comparison against production `main` reports **59 changed files** confined to `.github/`, `.gitignore`, `brand/`, `docs/`, `tools/`, and `ui-shell/`; there are **zero** changed paths under production `app/`, `wear/`, `control-plane/`, root `build.gradle.kts`, or root `settings.gradle.kts`.

A fresh source-graph audit at that exact head found **15 Kotlin source/test files**, **352 declared `ShellState` members**, **312 distinct UI `state.*` references**, no substantive unresolved state member references, **38 unique Compose instrumentation tests with no duplicate names**, **21 Settings destinations with 0 missing render routes and 0 orphan destinations**, **45 unique deterministic capture commands**, **0 Android permissions** in the shell manifest, **0 TODO/FIXME/XXX markers**, and no production-package/network/MediaStore/ContentResolver tokens in the checked shell Kotlin source.

The canonical type contract is now **TSUNAMI Sans v5.1 / Version 5.100**. The current manifest is `docs/TSUNAMI-SANS-v5.1-MANIFEST.json`; all build/source gates bind to that manifest, which records five masters at weights 300/400/500/600/700, 176 glyphs per master, Western-European/operational-symbol coverage, canonical proof hashes, and the proofed v5.1 generator lineage. The Android raster pass remains mandatory; desktop/Pillow proofing is not substituted for device rendering.

The public builder mirror in `brass-crusader-tx/chromagora-os:build/tsunami-ui-genesis-20260925` is rebound to this exact private head. Its `MIRROR-MANIFEST.json` names source head `46fd4b61915936187521a3e0c5f542505b315c48`; a fresh private-tree comparison checked **56 / 56** manifest-bound blobs (**40 ui-shell + 16 support**) with **0 mismatches**.
