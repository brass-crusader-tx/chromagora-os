...
@Composable fun OnboardingScreen(state:ShellState){
    ...
    Column(Modifier.fillMaxSize().statusBarsPadding().navigationBarsPadding().padding(28.dp)){
        ...
        OnboardChoice("Index a music folder","Local library · folders · offline",{state.banner="Sample folder indexed";state.onboardingComplete=true;state.primary=com.tsunami.shell.model.PrimarySpace.LIBRARY});OnboardChoice("Connect a music service","Catalogue + imported library provenance",{state.settingsExpanded="services"});OnboardChoice("Enter with the sample library","4,268 indexed items · no account",{state.banner="Sample library ready";state.onboardingComplete=true;state.primary=com.tsunami.shell.model.PrimarySpace.LISTEN});Spacer(Modifier.weight(1f));Text("PREVIEW LIBRARY · LOCAL SAMPLE STATE",style=Type.micro.copy(color=p.ink3))
    }
