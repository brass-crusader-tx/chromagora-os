...
fun main(){
 ...
 s.highContrast=true;s.largeControls=true;s.cycleHapticStrength();ok(s.hapticStrength==2,"accessibility state");s.cycleSwipeUp();ok(s.swipeUp=="Lyrics","swipe up");s.cycleSwipeDown();ok(s.swipeDown=="Queue","swipe down");s.cycleAlbumArtistMode();ok(s.albumArtistMode=="Track artist only","album artist");s.prepareDeviceMigration();ok(s.deviceMigrationStatus=="Ready to send","device migration")
